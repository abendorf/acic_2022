...
def impute_po(df, cat_vars, treatment_var, outcome_var, n_folds=5, drop_cols=[], n_estimators=500, learning_rate=0.05, reg_objective="regression_l1", groups=None):

    column_trans = ColumnTransformer(
        [('categories', OneHotEncoder(handle_unknown="ignore"), cat_vars)], remainder='passthrough')
    ...
    splits = group_kfold.split(df.drop(columns=[outcome_var]), df[outcome_var], groups=groups)

    test_po = []
    split_list = []
    for train_index, test_index in splits: 
        X_train = df.drop(columns=[outcome_var]).drop(columns=drop_cols).copy().iloc[train_index,:]
        print(f"{X_train.columns=}")
        y_train = df[outcome_var].copy().iloc[train_index]
        X_test = df.drop(columns=[outcome_var]).drop(columns=drop_cols).copy().iloc[test_index,:]
