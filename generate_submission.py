...
def generate_practice_summary_row_po(df, number):
    ...
    summary.to_csv(
        Path(practice_results_dir, f"practice_summary_{number}.csv"), index=False
    )
...
def generate_file_summary_df_po(df, treatment_var, outcome_var, number):
    ...
    naive_est = (
        df.query(f"{treatment_var}==1")[outcome_var].mean()
        - df.query(f"{treatment_var}==0")[outcome_var].mean()
    )
    print(naive_est)

    # Generate required rows: https://acic2022.mathematica.org/submissions
    summary_df_rows.append(
        generate_summary_row_po(df_treated, outcome_var, "Overall", "NA", "NA", number)
    )
    for year in [3, 4]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"year=={year}"), "Y", "Overall", "NA", year, number
            )
        )
    for level in [0, 1]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"X1=={level}"), "Y", "X1", level, "NA", number
            )
        )
    for level in ["A", "B", "C"]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"X2=='{level}'"), "Y", "X2", level, "NA", number
            )
        )
    for level in [0, 1]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"X3=={level}"), "Y", "X3", level, "NA", number
            )
        )
    for level in ["A", "B", "C"]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"X4=='{level}'"), "Y", "X4", level, "NA", number
            )
        )
    for level in [0, 1]:
        summary_df_rows.append(
            generate_summary_row_po(
                df_treated.query(f"X5=={level}"), "Y", "X5", level, "NA", number
            )
        )
...
def save_file_summary_po(df, treatment_var, outcome_var, number):
    generate_file_summary_df(df, treatment_var, outcome_var, number).to_csv(
