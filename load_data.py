...
def load_raw_data(number):
    ...
    practice_year = pd.read_csv(Path(data_dir, "practice_year", f"acic_practice_year_{number}.csv"))
    patient = pd.read_csv(Path(data_dir, "patient", f"acic_patient_{number}.csv"))
    patient_year = pd.read_csv(Path(data_dir, "patient_year", f"acic_patient_year_{number}.csv"))
...
def join_data(practice, practice_year, patient, patient_year):
    ...
    practice_joined_patient = pd.merge(practice_joined, patient, how='left', on=["id.practice"])
    all_joined = pd.merge(practice_joined_patient, patient_year, how='left', on=["id.patient", "year"])
...
def lag_variables(df):
    ...
    for col_name in tqdm(df.columns):
        ...
        if col_name not in id_vars and col_name != 'Z' and (col_name == 'Y' or 'V' in col_name or col_name=="n.patients"): 
            for i in range(1,4):
