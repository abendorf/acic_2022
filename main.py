...
for n in tqdm(range(1,3401)):
    ...
    df = impute_po(df, cat_vars, treatment_var, outcome_var, drop_cols=['id.practice', 'id.patient', 'post'], n_folds=2, reg_objective="regression_l1", groups=df['id.practice'])

    summary_df = generate_file_summary_df_po(df, treatment_var, outcome_var, number)
    summary_df.to_csv(Path(individual_results_dir, f"summary_{number}.csv"), index=False)
    print(summary_df)
