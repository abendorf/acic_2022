...
print(df.columns)
df.sort_values(
    ["dataset.num", "variable", "level", "year"], na_position="first"
).fillna("NA").to_csv(
    Path(submissions_dir, individual_submissions_filename), index=False
)
...
df.sort_values(["dataset.num", "id.practice"]).to_csv(
    Path(submissions_dir, practice_submissions_filename), index=False
)
